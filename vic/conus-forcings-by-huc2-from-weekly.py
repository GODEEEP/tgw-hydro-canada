...
input_files = '/vast/projects/godeeep/VIC/forcing/conus_tgw_1_16_deg_historical/tgw_wrf_historic_hourly_{}*'
...
def subset_file(year, output_dir, nthread):
    ...
    #forcing = xr.open_mfdataset(list_files, concat_dim = 'time', combine = 'nested', parallel = True)
    #forcing['spatial_ref'] = forcing['spatial_ref'].isel(time = 0) # 'spatial_ref' should have no dimension
    forcing_hourly = xr.open_mfdataset(list_files, concat_dim = 'time', combine = 'nested', parallel = True)
    forcing_hourly['spatial_ref'] = forcing_hourly['spatial_ref'].isel(time = 0) # 'spatial_ref' should have no dimension
    forcing = forcing_hourly.resample(time='6H').mean()
    forcing['PRECIP'] = forcing_hourly['PRECIP'].resample(time='6H').sum(skipna = False)
    ...
    input_csv = os.path.join('input', 'grid_ids_ca.csv')
    ...
    for lon, lat in grid_lonlats:
        ...
        forcing_fn = f'{point_dir}/forcings_6h_16thdeg_{point_id:07}_{lon:0.5f}_{lat:0.5f}_{year}.nc' # only 6-hourly time-step required
...
if __name__ == "__main__":
    ...
    output_dir = '/vast/projects/godeeep/VIC/forcing/inputs_1_16_deg_by_huc2/'
