...
import numpy as np
from run_vic_impi import read_params, modify_params
...
def run_calibration(path_csv, path_output, point_id):
  ...
  shutil.copy('vic_image_impi.exe', subpath_output)  # vic_image
  shutil.copy('run_vic_impi.py', subpath_output)
...
ModelExecutable  ./run_vic_impi.py
...
'''
  ...
  subprocess.run(['./vic_image_impi.exe', '-g', 'config_final.txt'], env={'OMP_NUM_THREADS': '1', **os.environ})  # vic_image
  ...
  if os.path.exists(vic_runoff):
    ...
    os.system(f'rm vic_image_impi.exe Ostrich')
  ...
  # remove binaries to save space
  os.system(f'rm vic_image_impi.exe Ostrich run_vic_impi.py')
